...
fitur_train = saved["columns"]      # SEKARANG 10 fitur
...
def load_model(method):
    ...
    if not os.path.exists(path):
        return None, f"Model '{method}' tidak ditemukan pada: {path}"
...
def predict_ml(method, fitur_list):
    ...
    if len(fitur_list) != len(fitur_train):
        return {
            "status": False,
            "error": f"Harus ada {len(fitur_train)} nilai fitur!"
        }
    ...
    try:
        ...
        for col, le in encoders.items():
            if col in df_input.columns:
                ...
                # Cegah value yang tidak pernah muncul saat training
                if val not in le.classes_:
                    return {
                        "status": False,
                        "error": f"Value '{val}' pada kolom '{col}' tidak valid. Pilihan: {list(le.classes_)}"
                    }

                df_input[col] = le.transform(df_input[col])

        df_input = df_input[fitur_train]  # Sesuaikan urutan

        pred = model.predict(df_input)[0].item()
