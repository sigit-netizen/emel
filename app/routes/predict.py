...
from flask import Blueprint, request, jsonify
from app.controllers.ml_controller import predict_ml
import datetime
...
def predict():
    ...
    try:
        ...
        if not method:
            return jsonify({"status": False, "error": "Parameter 'method' wajib diisi"}), 400
        if not isinstance(fitur, list):
            return jsonify({"status": False, "error": "Parameter 'fitur' harus berupa list/array"}), 400

        print(f"[INFO] Method: {method}, Fitur (list): {fitur}")
